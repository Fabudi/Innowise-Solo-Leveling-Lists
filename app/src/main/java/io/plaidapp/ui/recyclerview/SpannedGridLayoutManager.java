...
public class SpannedGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    @Keep /* XML constructor, see RecyclerView#createLayoutManager */ public SpannedGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SpannedGridLayoutManager, defStyleAttr, defStyleRes);
        ...
        // TODO use this!
        int orientation = a.getInt(R.styleable.SpannedGridLayoutManager_android_orientation, RecyclerView.VERTICAL);
        a.recycle();
        setAutoMeasureEnabled(true);
    }
    ...
    private int getNextSpannedRow(int rowIndex) {
        ...
        while (nextRow < getSpannedRowCount() && getFirstPositionInSpannedRow(nextRow) == firstPositionInRow) {
            nextRow++;
        }
        return nextRow;
    }
    ...
    private int getLastPositionInSpannedRow(final int rowIndex, RecyclerView.State state) {
        ...
        return (nextRow != getSpannedRowCount()) ? // check if reached boundary
                getFirstPositionInSpannedRow(nextRow) - 1 : state.getItemCount() - 1;
    }
    ...
    private int layoutRow(int rowIndex, int startTop, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int position = firstPositionInRow; position <= lastPositionInRow; position++, insertPosition++) {
            ...
            int wSpec = getChildMeasureSpec(cellBorders[cell.column + cell.columnSpan] - cellBorders[cell.column], View.MeasureSpec.EXACTLY, 0, lp.width, false);
            int hSpec = getChildMeasureSpec(cell.rowSpan * cellHeight, View.MeasureSpec.EXACTLY, 0, lp.height, true);
    ...
    private void calculateWindowSize() {
        ...
        int cellWidth = (int) Math.floor((getWidth() - getPaddingLeft() - getPaddingRight()) / columns);
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec) {
        ...
        widthSpec = updateSpecWithExtra(widthSpec, lp.leftMargin, lp.rightMargin);
        heightSpec = updateSpecWithExtra(heightSpec, lp.topMargin, lp.bottomMargin);
